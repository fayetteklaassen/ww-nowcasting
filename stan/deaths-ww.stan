...
model {
  ...
  if(pre_period_zero==1){
    if(N_weeks_before>0){
      ...
      if (sum(fitted_wastewater_prvl[1:N_weeks_before]) < 0)
        reject("`sum(fitted_wastewater_prvl[1:N_weeks_before])` had a negative value");

      target += neg_binomial_2_lpmf( 0 | sum(fitted_deaths[1:N_weeks_before]), phi_die);
      target += student_t_lpdf( 0 |10, sum(fitted_wastewater_prvl[1:N_weeks_before]), sigma);
    }
  ...
  if (fitted_wastewater_prvl[1] < 0)
        reject("`fitted_wastewater_prvl[1]` had a negative value");
    }
  }

  if (min(fitted_deaths) < 0)
    reject("`fitted_deaths` had a negative value");
  if (min(fitted_wastewater_prvl) < 0)
    reject("`fitted_wastewater_prvl` had a negative value");
  // LIKELIHOOD
  // During data
   ...
   for(i in 1:N_weeks){
    if(i < lastDeathWeek) {
    if(observed_ww[i] == 1){
    target += student_t_lpdf(obs_ww_mvs[i]|10, fitted_wastewater_prvl[N_weeks_before+i], sigma);
}   
 if(i > 4) 
